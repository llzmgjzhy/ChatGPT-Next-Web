...
export const useHomeworkStore = createPersistStore(
  DEFAULT_CHAT_STATE,
  (set, _get) => {
    function get() {
      return {
    ...
    const methods = {
      ...
      addUserMessagesFromSupabase(messages: ChatMessageSupabase) {
        ...
        get().updateCurrentSession((session) => {
          session.messages = session.messages.concat([userMessages]);
        });
      },

      addAssistantMessagesFromSupabase(messages: ChatMessageSupabase) {
        ...
        get().updateCurrentSession((session) => {
          session.messages = session.messages.concat([assistantMessages]);
        });
      },
